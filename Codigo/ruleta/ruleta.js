...
document.addEventListener('DOMContentLoaded', () => {
    const listavacia = document.querySelectorAll(".vacio");
    const angulo2 = 560 / listavacia.length;
    const radio2 = 600; // Radio del círculo en px
    let anguloTotal2 = 0; // Ángulo de rotación total  

    // Posiciona los números alrededor del centro de la ruleta
    listavacia.forEach((vacio, i) => {
        const angulo = i * angulo2;
        // La transformación mueve el número al borde del círculo y luego lo rota para que el texto esté recto
        vacio.style.transform = `rotate(${angulo2}deg) translate(${radio2/2}px) rotate(${-angulo2}deg)`;
        vacio.style.top = '50%'; // Centra verticalmente el contenedor
        vacio.style.left = '50%'; // Centra horizontalmente el contenedor
        vacio.style.transformOrigin = '0 0'; // Establece el origen de transformación en la esquina superior izquierda
    });

    // Función para actualizar la rotación de la ruleta
    function actualizarRotacion2() {
        anguloTotal2 -= 1; // Incrementa el ángulo para la rotación
        document.querySelector('.ruleta2').style.transform = `rotate(${-anguloTotal2}deg)`; 
    }

    // Usa setInterval para llamar a actualizarRotacion cada 16 ms (aproximadamente 60 FPS)
    setInterval(actualizarRotacion2, 15); // Ajusta el intervalo para cambiar la velocidad
});
